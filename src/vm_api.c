...
int API_GetMovieKeys() {
	...
	if (response_buffer == NULL) {
		LOG(ERROR, "[API] GetMovieKeys failed, unable to allocate memory");
		return -1;
	}
	...
	LOG(INFO, "[API] GetMovieKeys completed, size: %d", retlen);
	...
	if (fp) {
		...
		LOG(ERROR, "[API] GetMovieKeys failed, could not write keyblock to %s", f_keyblock);	
	}
	free(response_buffer);
        response_buffer = NULL;
	return -1;
...
retry:
	...
	// Get the Master Keys
	if(API_GetMovieKeys() < 0) {
		LOG(ERROR, "[API] GetMovieKeys failed");
		if(retry_count < 2){
			retry_count += 1;
